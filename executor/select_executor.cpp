...
bool Executor::AnalyzeSelectStatement(const hsql::SelectStatement *stmt,
  ...
  if (!Executor::dbmsContext->HasTable(query_tab)) {
    return false;
  }
...
std::vector<TableColumn> Executor::DecodeRowResult(std::string res,
                                                   std::string tab) {
  std::vector<std::string> cols = StringSplit(res, "$$");
  std::map<int, std::string> indexColMap =
      Executor::dbmsContext->GetCurDB()->GetTable(tab)->GetIndexColMap();
...
std::vector<std::vector<TableColumn> > Executor::SelectRowsFromPMemKV(
  ...
  if (scan_all) {
    ...
    std::string key = "data_" + dbName + "_" + query_tab + "_p_" + "*";
    redisReply *reply = (redisReply *)redisCommand(
        Executor::GetContext(), "SCAN 0 MATCH %s COUNT 100", key.c_str());
    ...
    case hsql::kOpGreater: {
      ...
      if (reply->element != nullptr) {
        ...
        for (int i = 0; i < reply->element[1]->elements;) {
          ...
          std::string pkey = reply->element[1]->element[i++]->str;
          std::string rowVal = reply->element[1]->element[i++]->str;
          if (pkey.compare(std::string("data_" + dbName + "_" + query_tab +
                                       "_p_" + query_value)) > 0) {
            auto row = DecodeRowResult(rowVal, query_tab);
            rows.push_back(std::move(row));
          }
...
bool Executor::SendResultSetToClient(
  ...
  for (int i = 0; i < fieldcount; ++i) {
    // get field from meta
    std::string colname = Executor::dbmsContext->GetCurDB()
                              ->GetTable(query_tab)
                              ->GetIndexColMap()[i];
