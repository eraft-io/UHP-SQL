...
struct ColumnDefinition : TableElement {
  ColumnDefinition(char* name, ColumnType type,
  ...
  void setNullableExplicit() {
    ...
    for (unsigned long constraint_index = 0;
         constraint_index < column_constraints->size(); constraint_index++) {
      if (column_constraints->at(constraint_index) == ConstraintType::Null) {
        nullable = true;
        column_constraints->erase(column_constraints->cbegin() +
                                  constraint_index);
      } else if (column_constraints->at(constraint_index) ==
...
// Example: "CREATE TABLE students (name TEXT, student_number INTEGER, city
// TEXT, grade DOUBLE)"
